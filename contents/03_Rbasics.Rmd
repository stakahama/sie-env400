...
* Write a function that accepts a subtable (a subset of a data table split according to variables passed to `group_by()`) and returns a different table. E.g., let us call this function `Foo()`.
* Pass the table to `do()` using this syntax: `do(Foo(.))`
    * `.` represents the table (data frame) itself
    * `Foo(.)` returns a data frame to be merged back with the other results
...
Check order of packages loaded into memory with `search()`. This list determines the order in which function definitions will be searched, much like the search path in MATLAB and your operating system.
