...
This example is shown mostly for illustrative purposes; such a relationship between radiation intensity and ozone concentration may be better captured by examining correlations between the daily maximum values of the two variables.
...
The corresponding correlation values can be obtained with the built-in function, `cor` (see `?cor). Note use of `use="pairwise.complete.obs"` for calculation of correlation with missing values.
