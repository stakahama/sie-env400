...
These properties have consequences for conclusions we draw from hypothesis testing (and problems in statistical inference in general). For descriptive statistics, high variance of an estimator may be less of an issue (though the deviation of any single estimate from $\theta$ may be greater).
...
```{r}
pm10.zue <- lf %>%
  filter(site=="ZUE" & variable=="PM10") %>%
  mutate(date = format(dates(datetime), "y.m.d"),
...
pm10.zue %>%
  mutate(day.of.month=day+hour/24) %>%
  ggplot+
...
ggplot(highdays, aes(hour, value))+
  geom_line()+
  geom_point()+
  facet_grid(.~date)+
  xlab("Hour of day")+
...
```{r, fig.width=15, fig.height=3}
filter(pm10.zue, date %in% exceeded[["date"]]) %>%
...
```{r}
ggplot(anomolous.day, aes(hour, value))+
  geom_line()+
  geom_point()+
